...
public class ReviewDaoImplTest {
    ...
    public void testAddUpdateGetReview() {
        ...
        int review1Id = reviewDao.add(review1).getReviewId();
        Review testReviewAfterAdd = reviewDao.findReviewByID(review1Id);
        ...
        int review2Id = reviewDao.add(review2).getReviewId();
        Review review3 = new Review(0, "ReviewUser1Film2", false, user1Id, film2Id, 0);
        int review3Id = reviewDao.add(review3).getReviewId();
        List<Review> reviewList = reviewDao.findAllReviews(10);
        assertThat(reviewList.size()).isEqualTo(3);
        reviewList = reviewDao.findReviewsByFilmID(film1Id, 10);
        assertThat(reviewList.size()).isEqualTo(2);
        reviewList = reviewDao.findReviewsByFilmID(film2Id, 10);
        assertThat(reviewList.size()).isEqualTo(1);
        /* Получаем отзывы с ограничением количества */
        reviewList = reviewDao.findAllReviews(2);
        assertThat(reviewList.size()).isEqualTo(2);
        ...
        Review testReviewAfterUpdate = reviewDao.findReviewByID(review1Id);
        ...
        reviewDao.update(updatedReviewWithBadId);
        Review testReviewAfterBadUpdate = reviewDao.findReviewByID(999);
        ...
        final UnregisteredDataException exceptionNotUser = assertThrows(
                UnregisteredDataException.class, () -> reviewDao.add(reviewFromNotExistUser));
        ...
        final UnregisteredDataException exceptionNotFilm = assertThrows(
                UnregisteredDataException.class, () -> reviewDao.add(reviewFromNotExistFilm));
